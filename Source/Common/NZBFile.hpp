...
private:
	...
	boost::filesystem::path m_cache_diskfile;
	yCacheFile m_cache_file;
	...
	std::pair<std::size_t, std::size_t> FindSegmentIdxRange(const unsigned long long begin, const unsigned long long end);
	
	std::size_t TryGetData(char* buf, const unsigned long long offset, const std::size_t size, bool& err,
	...
	struct AsyncDataRequest
	{
		...
		AsyncDataRequest(uint_fast64_t i, OnDataFunction f, char* b, const unsigned long long o, const std::size_t s,
			boost::signals2::connection c, const bool prio) :id(i), func(f), buf(b), offset(o), size(s), conn(c), priority(prio)
		{}
	};

	std::list< std::shared_ptr<AsyncDataRequest> > m_data_requests;
	...
	std::list< boost::shared_ptr<AsyncFilenameRequest> > m_filename_requests;
public:
...
//	OnFilenameHandle OnFilename;
	...
	void SetDownloadFinished(const std::size_t idx);
	...
	struct MyDecoder : public yDecoder::Callbacks
	{
		...
		yCacheFileHandle OnBeginSegment(const yBeginInfo& beginInfo, const yPartInfo& partInfo);
		yCacheFileHandle OnBeginSegment(const yBeginInfo& beginInfo);
	...
	void Decode(boost::shared_ptr<MyDecoder> decoder, const bool code, const std::string& article_data);
