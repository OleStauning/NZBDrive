...
public:
	...
	RARFile(Logger& log, const std::filesystem::path& path, const std::filesystem::path& filename, const rar::file_head& file_head);
	virtual ~RARFile();	
...
class RARFileFactory
{
	...
	typedef std::map<std::pair<std::filesystem::path,std::string>, std::shared_ptr<RARFile> > RARFileMap;
