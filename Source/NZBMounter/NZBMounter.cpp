...
std::filesystem::path expand_user(std::filesystem::path path)
{
	...
	std::filesystem::path homedrivepath;
	if (homedrive != nullptr) homedrivepath = homedrivepath / homedrive;
	homedrivepath = homedrivepath / home;
	std::string s = path.string();
	if (s[0] == '~') 
	{
		s = homedrivepath.string() + s.substr(1, s.size() - 1);
...
int main(int argc, char *argv[])
{
	...
	auto configpath = expand_user(std::filesystem::path() / "~" / ".nzbmount");
	auto configfile = configpath / "config";
	...
	if (!std::filesystem::exists(configfile))
	{
		...
		cf.close();
		std::cout << "Edit the configuration file \""<<configfile<<"\" and try again" << std::endl;
		return 1;
	}
